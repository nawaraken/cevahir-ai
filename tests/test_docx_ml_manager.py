...
from modules.docx_machine_learning_manager import DocxMachineLearningManager
from config.parameters import (PROCESSED_DATA_DIR, DOCX_VOCAB_SAVE_PATH, MIN_FREQ, 
                               MAX_VOCAB_SIZE, NUM_RANDOM_DOCX, MAX_TOKENS, STOPWORDS, 
                               SPECIAL_TOKENS, INPUT_DIM, MODEL_TRAINING_DATA_DIR)

class TestDocxMachineLearningManager(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        # Tokenizer, VocabManager ve MachineLearningManager'ı oluştur
        ...
        cls.ml_manager = DocxMachineLearningManager()

        # Processed data directory yoksa oluştur
        os.makedirs(PROCESSED_DATA_DIR, exist_ok=True)
        os.makedirs(MODEL_TRAINING_DATA_DIR, exist_ok=True)
        ...
        # Kelime hazinesi oluştur ve kaydet
        cls.vocab_manager.process_docx_data(cls.processed_data)
        cls.vocab_utils.save_vocab_to_file(cls.vocab_manager.load_existing_vocab(), DOCX_VOCAB_SAVE_PATH)
    ...
    def test_machine_learning_preparation(self):
        """Makine diline çevirme sürecinin doğru şekilde yapıldığını test eder."""
        self.ml_manager.run_training_data_preparation(DOCX_VOCAB_SAVE_PATH)

        # İşlenen veriyi dosyadan yükle
        prepared_data_path = os.path.join(MODEL_TRAINING_DATA_DIR, f"docx_training_data_dim_{INPUT_DIM}.npy")
        self.assertTrue(os.path.exists(prepared_data_path), "Makine öğrenmesi için hazırlanan veri dosyası mevcut olmalıdır.")

        # Veriyi yükleyip biçimini kontrol et
        loaded_data = np.load(prepared_data_path)
        self.assertIsInstance(loaded_data, np.ndarray, "Hazırlanan veri, np.ndarray formatında olmalıdır.")
        self.assertGreater(loaded_data.size, 0, "Hazırlanan veri boş olmamalıdır.")
        logging.info("Makine diline çevirme süreci başarıyla test edildi.")
